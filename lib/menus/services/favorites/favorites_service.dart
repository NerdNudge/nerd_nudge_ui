...
class FavoritesService {
  ...
  Future<List<Map<String, dynamic>>> getRecentFavorites() async {
    ...
    dynamic result;
    try {
      final String url = "${APIEndpoints.USER_INSIGHTS_BASE_URL}${APIEndpoints.RECENT_FAVORITES}/" + UserProfileEntity().getUserEmail();
      NerdLogger.logger.d('Sending GET request to: $url');
      result = await apiService.getRequest(APIEndpoints.USER_INSIGHTS_BASE_URL, "${APIEndpoints.RECENT_FAVORITES}/" + UserProfileEntity().getUserEmail());
  ...
  Future<dynamic> favoritesActivitySubmission(FavoriteUserActivityEntity entity) async {
    ...
    dynamic result;
    try {
      const String url = APIEndpoints.USER_ACTIVITY_BASE_URL + APIEndpoints.FAVORITES_SUBMISSION;
